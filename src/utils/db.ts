...
import { HealthKitWorkout, HistoricalHealthData } from "@/types/HealthData";
  ...
  item:
    ...
    | {
        timestamp:
          | string
          | Date
          | Prisma.DateTimeFilter<"HealthDataPoint">
          | undefined;
        category: string;
        userId: number;
      }
 ...
 */
async function getWorkouts(userId: number): Promise<Workout[]> {
...
async function getSleepSessions(userId: number): Promise<SleepSession[]> {
  return [];
}
 ...
 */
async function getLayouts(userId: number): Promise<TypedLayout[]> {
 ...
 */
async function createLayout(userId: number, data: TypedLayout): Promise<TypedLayout> {
  const rec = await prisma.layout.create({
    data: {
      userId,
      name: data.name,
...
async function updateLayout(
  ...
  const rec = await prisma.layout.update({
 ...
 * Throws an error if the layout is not found or does not belong to the user.
 */
async function deleteLayout(userId: number, layoutId: number): Promise<void> {
  const existing = await DBAdapter.getPrismaClient().layout.findFirst({
    where: { id: layoutId, userId },
  });
  if (!existing) {
 ...
 */
async function getCounters(userId: number): Promise<Counter[]> {
 ...
 */
async function createCounter(userId: number, data: Counter): Promise<Counter> {
